        ...
        try:
            ...
            if 'Select a Merchant and Marketplace'in driver.page_source:
                ...
                try:
                    ...
                    num="3"
                    driver.find_element(By.XPATH,
                                        f'//*[@id="picker-container"]/div/div[{num}]/div/button').click()
                    sleep(999)
                except:
                    pass
                ...
                # sleep(999)
                # sleep(999)
                ''' <kat-alert variant="danger" size="large"><span slot="header">आंतरिक विफलता</span><span>क्षमा करें, हम आपके अनुरोध को प्रोसेस नहीं कर पाए. पुन: प्रयास करें.</span></kat-alert> '''
                '''<kat-alert variant="danger" size="large" dismissed=""><span slot="header">आंतरिक विफलता</span><span>क्षमा करें, हम आपके अनुरोध को प्रोसेस नहीं कर पाए. पुन: प्रयास करें.</span></kat-alert>'''
                #//*[@id="root"]/div/kat-alert
                for i in range(0, int(numofcall)):
                    if 'Internal failure' in driver.page_source\
                        or 'આંતરિક નિષ્ફળતા' in driver.page_source\
                        or 'आंतरिक विफलता' in driver.page_source\
                        or 'ಆಂತರಿಕ ವೈಫಲ್ಯ' in driver.page_source\
                        or 'உள்ளக தோல்வி' in driver.page_source\
                        or 'Interner Fehler' in driver.page_source\
                        or 'Falha interna' in driver.page_source\
                        or '内部エラー' in driver.page_source\
                        or 'Error interno' in driver.page_source\
                        or 'Errore interno' in driver.page_source\
                        or 'Erreur interne' in driver.page_source\
                        or 'Error interno' in driver.page_source\
                        or 'Interne storing' in driver.page_source:

                        if 'The captcha you entered was incorrect' in driver.page_source\
                        or 'તમે દાખલ કરેલ કેપ્ચા ખોટી હતી. કૃપા કરીને ફરીથી પ્રયાસ કરો.' in driver.page_source\
                        or 'आपके द्वारा डाला गया कैप्चा गलत है. फिर से कोशिश करें.' in driver.page_source\
                        or 'ನೀವು ನಮೂದಿಸಿದ ಕ್ಯಾಪ್ಚಾ ತಪ್ಪಾಗಿದೆ. ದಯವಿಟ್ಟು ಮತ್ತೊಮ್ಮೆ ಪ್ರಯತ್ನಿಸಿ.' in driver.page_source\
                        or 'நீங்கள் உள்ளிட்ட கேப்ட்சா தவறானது. மீண்டும் முயலவும்.' in driver.page_sourc\
                        or 'Das eingegebene Captcha ist nicht korrekt. Bitte versuchen Sie es erneut.' in driver.page_source\
                        or '入力された認証情報が正しくありません。もう一度お試しください。' in driver.page_source\
                        or 'El captcha que ingresaste es incorrecto. Vuelve a intentarlo.' in driver.page_source\
                        or 'Il captcha inserito non era corretto. Riprova.' in driver.page_source\
                        or 'Le captcha saisi est incorrect. Veuillez réessayer.' in driver.page_source\
                        or 'El captcha introducido no es correcto. Inténtalo de nuevo.' in driver.page_source\
                        or 'De captcha die je hebt ingevoerd, was onjuist. Probeer het nogmaals.' in driver.page_source\
                        or 'Não foi possível processar a solicitação. Tente novamente.' in driver.page_source:
                            driver.refresh()
                            pass
                        else:
                            print('failure')
                    ...
                    for j in range(0, int(numofcallForNumber)):
                        try:
                            ...
                            # if "Select a language from the drop-down list"in driver.page_source:
                            #     element=WebDriverWait(driver,5).until(lambda driver:
                            #         driver.find_element(
                            #             by=By.CSS_SELECTOR,
                            #             value='#root > div > form > div > div.hill-language-switcher > kat-dropdown'))
                            #     shadow_root1 = expand_shadow_element(element, driver)
                            #     sleep(0.3)
                            #     shadow_root1.find_element(By.ID,
                            #                         value='katal-id-0').click()
                            #     country=open("country.txt",'r').readline()
                            #     shadow_root1.find_element(By.CSS_SELECTOR,value=f'div.kat-select-container > div.select-options > div > div > slot > kat-option:nth-child({country})').click()
                            #
